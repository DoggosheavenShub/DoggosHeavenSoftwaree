...
router.post("/addvisittype", protectedRoute, async (req, res) => {
  try {
    ...
    const { purpose, price, halfdayprice, emoji, description, isSubscriptionAvailable, subscriptionPrice, consultationPricePvt, customFields } = req.body;
    if (!purpose) return res.status(400).json({ success: false, message: "Purpose is required" });
    const vt = await VisitType.create({ purpose, price, halfdayprice, emoji: emoji || "🐾", description: description || "", isSubscriptionAvailable: isSubscriptionAvailable || false, subscriptionPrice: subscriptionPrice || null, consultationPricePvt: consultationPricePvt || null, customFields: customFields || [] });
...
router.put("/updatevisittype/:id", protectedRoute, async (req, res) => {
  try {
    const VisitType = require('../models/visitTypes');
    const { purpose, price, halfdayprice, emoji, description, isSubscriptionAvailable, subscriptionPrice, consultationPricePvt, customFields } = req.body;
    const vt = await VisitType.findByIdAndUpdate(req.params.id, { purpose, price, halfdayprice, emoji, description, isSubscriptionAvailable, subscriptionPrice, consultationPricePvt, customFields }, { new: true });
