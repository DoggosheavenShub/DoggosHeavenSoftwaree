...
const VisitHistoryDetails = ({ visitdetails, onClose }) => {
  ...
  useEffect(() => {
    const fetchVeterinaryVisitDetail = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(`Failed to fetch visit details. Status: ${response.status}`);
        }

        const data = await response.json();
        console.log("Veterinary visit detail:", data);
  ...
  useEffect(() => {
    const fetchBuySubscriptionDetail = async () => {
      ...
      try {
        setLoading(true);
        console.log("Fetching subscription visit detail for ID:", visitdetails._id);
    ...
    return (
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-4 text-gray-700 text-sm">
          ...
          <div><span className="font-semibold">Owner:</span> {details?.pet?.owner?.name || ""}</div>
          <div><span className="font-semibold">Phone:</span> {details?.pet?.owner?.phone || ""}</div>
          <div><span className="font-semibold">Purpose:</span> {details?.visitType?.purpose || ""}</div>
        </div>

        <div className="mt-4 bg-blue-50 rounded-lg p-4 border border-blue-200">
          <h3 className="text-base font-bold text-blue-800 mb-2">🏠 Hostel Details</h3>
          <div className="grid grid-cols-2 gap-4 text-gray-700 text-sm">
            <div><span className="font-medium">Stay Duration:</span> {details?.details?.numberOfDays || "0"} days</div>
            <div><span className="font-medium">Price:</span> ₹{details?.details?.price || "0"}</div>
            <div><span className="font-medium">Daily Rate:</span> ₹{details?.visitType?.price || "0"}</div>
            {details?.visitType?.halfdayPrice && (
              <div><span className="font-medium">Half-day Rate:</span> ₹{details?.visitType?.halfdayPrice}</div>
            )}
            <div><span className="font-medium">Discount Applied:</span> ₹{details?.details?.discount || "0"}</div>
            <div><span className="font-medium">Subscription Used:</span> {details?.details?.isSubscriptionAvailed ? "Yes" : "No"}</div>
          </div>
        </div>

        {details?.details?.medicines && details.details.medicines.length > 0 && (
          <div className="mt-4">
            <h3 className="text-base font-bold text-gray-800 mb-2">💊 Medicines Given During Stay</h3>
            <div className="space-y-2">
              {details.details.medicines.map((med, index) => (
                <div
                  key={index}
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="mt-4 bg-green-50 rounded-lg p-4 border border-green-200">
          <h3 className="text-base font-bold text-green-800 mb-2">🔄 Subscription Details</h3>
          <div className="grid grid-cols-2 gap-4 text-gray-700 text-sm">
            <div><span className="font-medium">Transaction Type:</span> {details?.details?.purpose || details?.visitType?.purpose || "Buy Subscription"}</div>
            ...
            {subscriptionTypeValue ? (
              ...
              <div className="col-span-2">
                ...
                <div>
                  <span className="font-medium">Amount Paid:</span> ₹{details?.details?.price || "0"}
                </div>
              </div>
            )}
            
            <div><span className="font-medium">Purchase Date:</span> {visitDate}</div>
            
            {/* If this is a plan renewal */}
            {(details?.details?.purpose?.toLowerCase()?.includes('renew') || 
              details?.visitType?.purpose?.toLowerCase()?.includes('renew')) && (
    ...
    return (
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-4 text-gray-700 text-sm">
          ...
          <div><span className="font-semibold">Owner:</span> {details?.pet?.owner?.name || ""}</div>
          <div><span className="font-semibold">Phone:</span> {details?.pet?.owner?.phone || ""}</div>
          <div><span className="font-semibold">Total Amount:</span> ₹{details?.details?.price || "0"}</div>
        </div>
  
        <div className="mt-4 bg-purple-50 rounded-lg p-4 border border-purple-200">
          <h3 className="text-base font-bold text-purple-800 mb-2">🛍️ Shopping Details</h3>
          
          {details?.details?.items && details.details.items.length > 0 ? (
            <div className="mt-2">
              <h4 className="text-sm font-bold text-gray-700 mb-2">Products Purchased</h4>
              <div className="space-y-2">
                {details.details.items.map((item, index) => (
                  ...
                  >
                    <div><span className="font-medium">Product Name:</span> {item.name || "Unknown Product"}</div>
                    {item.quantity && (
                      <div><span className="font-medium">Quantity:</span> {item.quantity || "1"} {item.unit || 'units'}</div>
                    )}
                    {item.price && (
            ...
            <div className="text-gray-500 italic">No product details available</div>
          )}
        </div>
      </div>
    );
